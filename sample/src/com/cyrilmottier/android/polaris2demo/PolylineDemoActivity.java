...
public class PolylineDemoActivity extends FragmentActivity implements OnSeekBarChangeListener {
    ...
    private void setUpMap() {
        ...
        // A simple polyline with the default options wrap Melbourne-Adelaide-Perth.
        mMap.addPolyline((new PolylineOptions()).add(MELBOURNE, ADELAIDE, PERTH));

        // A geodesic polyline that goes around the world.
        mMap.addPolyline((new PolylineOptions()).add(LHR, AKL, LAX, JFK, LHR).width(5).color(
        ...
        int color = Color.HSVToColor(mAlphaBar.getProgress(),
                new float[]{mColorBar.getProgress(), 1, 1});
        mMutablePolyline = mMap.addPolyline(options.color(color).width(mWidthBar.getProgress()));
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        ...
        if (seekBar == mColorBar) {
            mMutablePolyline.setColor(Color.HSVToColor(Color.alpha(mMutablePolyline.getColor()),
                    new float[]{progress, 1, 1}));
